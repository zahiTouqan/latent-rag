...
def load_hidden_states(path: str, device: str) -> Tuple[Tensor, Tensor]:
    data = torch.load(path, map_location=device)
    return data["hidden_states"].to(device), data["attention_mask"].to(device)
...
def combine_chunk_states(
    chunks: List[Dict[str, str]],
    device: str,
) -> Tuple[Tensor, Tensor]:
    """
    Loads top-k chunk hidden states and concatenates them along sequence dimension.

    Returns:
        combined_hidden_states: [1, total_seq_len, hidden_dim]
        combined_attention_mask: [1, total_seq_len]
    """
    hidden_state_list: List[Tensor] = []
    attention_mask_list: List[Tensor] = []

    for chunk in chunks:
        hidden_states, attention_mask = load_hidden_states(chunk["state_path"], device)
        hidden_state_list.append(hidden_states)     # [1, seq_len, hidden_dim]
        attention_mask_list.append(attention_mask)  # [1, seq_len]

    combined_hidden_states: Tensor = torch.cat(hidden_state_list, dim=1)
    combined_attention_mask: Tensor = torch.cat(attention_mask_list, dim=1)

    return combined_hidden_states, combined_attention_mask
...
def main() -> None:
    ...
    combined_hidden_states, combined_attention_mask = combine_chunk_states(
        chunks=top_chunks,
        device=device,
    )

    print("\nCombined latent context:")
    print(f"Hidden states shape: {combined_hidden_states.shape}")
    print(f"Attention mask shape: {combined_attention_mask.shape}")

    encoder_outputs = BaseModelOutput(last_hidden_state=combined_hidden_states)

    decoder_prompt: str = query
    decoder_input_ids: Tensor = tokenizer(
        decoder_prompt,
        return_tensors="pt",
    ).input_ids.to(device)
